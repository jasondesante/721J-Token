...
import "./Interfaces/IERC721J.sol"; // Interface Id: 0x75b86392
import "./Interfaces/IERC721JFull.sol"; // Interface Id: 0xa360e0cd
import "./Interfaces/IERC721JPromo.sol"; // Interface Id: 0x17bbaa28
import "./Interfaces/IERC721JEnumerable.sol"; // Interface Id: 0xd2ac8720 

import "./Interfaces/IERC2981.sol"; // Interface Id: 0x2a55205a
    ...
    IERC2981
{
    ...
    function setPromo(
        ...
        uint256 _songOut = (promoPercentOut * (10**36)) + (songIdOut * (10**18)) + generationOut;
    ...
    function promoCheckSpecific(
        ...
        uint256 songIdIn,
        uint256 generationIn
    ) public view virtual override returns (uint256 _songIdOut, uint256 _generationOut, uint256 _promoPercentOut) {
        uint256 _songIn = (songIdIn * (10**18)) + generationIn;
    ...
    function payPromo(uint256 _songOut, uint256 _songIdOut) internal {
        ...
        // Requires eth
        if (msg.value < (_mintPrice * _promoPercentOut) / 100) revert NotEnoughEther();
        // Transfer eth
        if (_promoPercentOut != 0 && address(_songSplits[_songIdOut]) != address(0))
    ...
    function promoMint(address _contract, uint256 tokenId) public payable override {
    ...
    function promoMintSpecific(address _contract, uint256 tokenId) public payable override {
        // Sets contract address as external nft
        ERC721J _claimContract = ERC721J(_contract);
        // Checks if the token has been claimed
    ...
    function promoMintSong(address _contract, uint256 tokenId) public payable {
        // Sets contract address as external nft
        ERC721J _claimContract = ERC721J(_contract);
        // Checks if the token has been claimed
        if (_tokenClaimSpecific[_contract][tokenId] != false)
    ...
    function promoMintRarity(address _contract, uint256 tokenId) public payable {
        // Sets contract address as external nft
        ERC721J _claimContract = ERC721J(_contract);
        // Checks if the token has been claimed
        if (_tokenClaimSpecific[_contract][tokenId] != false)
